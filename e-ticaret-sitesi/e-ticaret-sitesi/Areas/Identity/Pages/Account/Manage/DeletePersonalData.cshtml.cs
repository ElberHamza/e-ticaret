...
namespace e_ticaret_sitesi.Areas.Identity.Pages.Account.Manage
{
    public class DeletePersonalDataModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostAsync()
        {
            ...
            if (!result.Succeeded)
            {
                throw new InvalidOperationException("Kullanıcı silinirken beklenmeyen bir hata oluştu.");
            }

            await _signInManager.SignOutAsync();

            _logger.LogInformation("ID '{UserId}' olan kullanıcı hesabını sildi.", userId);
